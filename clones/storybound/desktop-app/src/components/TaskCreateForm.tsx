  ...
  return (
    <>
      ...
      <section className="builder-card">
        ...
        <label className="field-group"><span className="field-label field-label--standalone">提示词模板 <small>系统模板 / 我的模板</small></span><select className="text-input" value={form.promptTemplateId} onChange={(event) => updatePromptTemplate(event.target.value)}>{contentTracks.map((track) => <option key={track} value={`system-${track}`}>系统 · {track}</option>)}{customPromptTemplates.length ? <optgroup label="我的模板">{customPromptTemplates.map((template) => <option key={template.id} value={template.id}>{template.name} · {template.baseTrack}</option>)}</optgroup> : null}</select><span className="template-hint">{form.promptTemplateOverride ? `当前使用本地模板“${form.promptTemplateOverride.name}”，四段提示词会随任务保存并由流水线实际调用。` : "使用从原版 v1.16.1 提取的当前赛道提示词。"}</span></label>
      </section>
