  ...
  return (
    <>
      ...
      <section className="builder-card">
        ...
        {form.voiceSource === "tts" && form.videoForm === "narration" ? <div className="field-group"><span className="field-label field-label--standalone">旁白结构</span><div className="choice-grid choice-grid--two"><button type="button" className={`choice-card ${form.ttsMode === "original-segmented" ? "is-selected" : ""}`} onClick={() => onChange({ ttsMode: "original-segmented" })}><span className="choice-card__radio" /><span><strong>原版复刻 · 逐镜 TTS</strong><small>每镜独立图片、音频和字幕，按 MP3 实测时长对齐</small></span></button><button type="button" className={`choice-card ${form.ttsMode === "continuous" ? "is-selected" : ""}`} onClick={() => onChange({ ttsMode: "continuous" })}><span className="choice-card__radio" /><span><strong>连续旁白 · 增强模式</strong><small>整篇一次合成，减少音色重启；不是原版任务结构</small></span></button></div></div> : null}
