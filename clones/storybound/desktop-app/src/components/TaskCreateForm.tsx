  ...
  return (
    <>
      ...
      <section className="builder-card">
        ...
        <div className="field-group form-grid form-grid--two"><label><span className="field-label field-label--standalone">目标视频时长（秒）</span><NumberField value={form.targetDurationSec} placeholder="按配音决定" onChange={(targetDurationSec) => onChange({ targetDurationSec })} /></label><div><span className="field-label field-label--standalone">画面比例</span><div className="segmented-control">{ratios.map((ratio) => <button key={ratio} type="button" className={form.aspectRatio === ratio ? "is-selected" : ""} onClick={() => onChange({ aspectRatio: ratio })}>{ratio}</button>)}</div></div></div>
        ...
        <div className="field-group form-grid form-grid--two"><label><span className="field-label field-label--standalone">剪映草稿模板</span><select className="text-input" value={form.draftTemplateId} onChange={(event) => onChange({ draftTemplateId: event.target.value })}><option value="default-portrait-9-16">默认竖屏</option><option value="builtin-portrait-4-3">竖屏 4:3</option><option value="builtin-landscape-16-9">横屏 16:9</option><option value="builtin-knowledge-card">知识卡（分栏）</option></select></label><label className="upload-tile upload-tile--compact"><input type="file" accept="image/*" onChange={(event) => event.target.files?.[0] && onUploadReference(event.target.files[0])} /><strong>{referenceName ? "✓ 人物参考图已保存" : "人物参考图"}</strong><span>{referenceName || "上传后写入任务人物一致性配置"}</span></label></div>
