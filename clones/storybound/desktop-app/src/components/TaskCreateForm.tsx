...
export function TaskCreateForm({ form, voices, hasLlmCredentials, hasTtsCredentials, aiGenerating, taskReady, referenceName, externalAudioName, bgmName, onChange, onGenerateCopy, onUploadImages, onUploadReference, onUploadExternalAudio, onUploadBgm }: TaskCreateFormProps) {
    ...
    <>
      <section className="builder-card">
        ...
        <div className="field-group"><span className="field-label field-label--standalone">视频形式</span><div className="choice-grid choice-grid--two">
          {[{ value: "narration" as const, title: "旁白视频", description: "逐镜单人配音" }, { value: "podcast" as const, title: "双人播客", description: "[A] / [B] 双音色对谈" }].map((item) => <button key={item.value} type="button" className={`choice-card ${form.videoForm === item.value ? "is-selected" : ""}`} onClick={() => onChange({ videoForm: item.value })}><span className="choice-card__radio" /><span><strong>{item.title}</strong><small>{item.description}</small></span></button>)}
        </div></div>
      ...
      <section className="builder-card">
        ...
        <div className="field-group option-checks"><label><input type="checkbox" checked={form.dynamicStoryboard} onChange={(event) => onChange({ dynamicStoryboard: event.target.checked })} />动态分镜（按语义决定镜头数量）</label><label><input type="checkbox" checked={form.autoBorrowImage} onChange={(event) => onChange({ autoBorrowImage: event.target.checked })} />失败图自动使用相邻画面补位</label><label><input type="checkbox" checked={form.bgmSync} onChange={(event) => onChange({ bgmSync: event.target.checked })} />按 BGM 节奏卡点</label></div>
        ...
        <div className="field-group form-grid form-grid--two"><label><span className="field-label field-label--standalone">剪映草稿模板</span><select className="text-input" value={form.draftTemplateId} onChange={(event) => onChange({ draftTemplateId: event.target.value })}>{draftTemplates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></label><label className="upload-tile upload-tile--compact"><input type="file" accept="image/*" onChange={(event) => event.target.files?.[0] && onUploadReference(event.target.files[0])} /><strong>{referenceName ? "✓ 人物参考图已保存" : "人物参考图"}</strong><span>{referenceName || "上传后写入任务人物一致性配置"}</span></label></div>
        <div className="template-summary"><div><strong>{selectedTemplate.name}</strong><span>{selectedTemplate.config.canvas.width}×{selectedTemplate.config.canvas.height} · 画面 {selectedTemplate.config.image.ratio}</span></div><div><span>正文字幕 {selectedTemplate.config.caption.fontSize} 号 / 每行 {selectedTemplate.config.caption.maxCharsPerLine} 字</span><span>{selectedTemplate.config.caption.color} · 背景透明度 {selectedTemplate.config.caption.background.alpha}</span></div><div><span>免责声明 {selectedTemplate.config.disclaimer.visible ? "开启" : "关闭"}</span><span>旁白 {selectedTemplate.config.audio.narrationVolume} / BGM {selectedTemplate.config.audio.bgmVolume}</span></div></div>
        <div className="field-group form-grid form-grid--two"><label className="toggle-row"><input type="checkbox" checked={form.videoIntro} onChange={(event) => onChange({ videoIntro: event.target.checked })} />启用视频片头素材</label><label><span className="field-label field-label--standalone">片头素材时长（秒）</span><input className="text-input" type="number" min="1" max="15" disabled={!form.videoIntro} value={form.videoIntroDuration} onChange={(event) => onChange({ videoIntroDuration: Math.max(1, Math.min(15, Number(event.target.value) || 3)) })} /></label></div>
        <p className="template-hint">剪映标题、副标题由所选模板生成并覆盖全片；视频片头是原版的独立素材选项，不控制标题轨和口播字幕位置。</p>
        <div className="field-group form-grid form-grid--three"><div><span className="field-label field-label--standalone">封面海报</span><div className="segmented-control">{[["off", "关闭"], ["titled", "标题封面"], ["plain", "纯画面"]].map(([value, label]) => <button key={value} type="button" className={form.coverMode === value ? "is-selected" : ""} onClick={() => onChange({ coverMode: value as BuilderFormState["coverMode"] })}>{label}</button>)}</div></div><label><span className="field-label field-label--standalone">封面模板</span><select className="text-input" disabled={form.coverMode === "off"} value={form.coverTemplateId} onChange={(event) => onChange({ coverTemplateId: event.target.value })}><option value="cinematic-poster">电影海报</option><option value="editorial">杂志封面</option><option value="minimal">极简标题</option></select></label><label className="toggle-row toggle-row--bottom"><input type="checkbox" disabled={form.coverMode === "off"} checked={form.secondCover} onChange={(event) => onChange({ secondCover: event.target.checked })} />额外生成第二封面</label></div>
      </section>

      <section className="builder-card">
        <div className="builder-card__heading"><span className="builder-card__icon">♫</span><div><h2>配音</h2><p>逐镜 TTS、外部音频、双人播客与 BGM</p></div></div>
