...
export function TaskCreateForm({ form, voices, hasLlmCredentials, hasTtsCredentials, aiGenerating, taskReady, referenceName, coverLocalName, externalAudioName, bgmName, voicePreview, previewingVoiceId, previewingVoiceMode, voicePreviewError, onPreviewVoice, onChange, onGenerateCopy, onUploadImages, onUploadReference, onUploadCover, onUploadTemplateBackground, onUploadExternalAudio, onUploadBgm }: TaskCreateFormProps) {
  ...
  return (
    <>
      ...
      <section className="builder-card">
        ...
        <span className="field-label field-label--standalone">素材来源</span><div className="choice-grid choice-grid--three">{[["ai", "AI 绘图", "按画面风格生成插画 / 写实图"], ["stock", "网络素材", "自动检索真实图片并保存授权信息"], ["local", "我的素材库", "用你收集的真实照片"]].map(([value, title, description]) => <button key={value} type="button" className={`choice-card choice-card--compact ${form.materialSource === value ? "is-selected" : ""}`} onClick={() => onChange({ materialSource: value as BuilderFormState["materialSource"] })}><span className="choice-card__radio" /><span><strong>{title}</strong><small>{description}</small></span></button>)}</div>
        ...
        })}</div>{form.dynamicStoryboard && ![3, -1].includes(form.videoIntroCount) ? <label><span className="field-label field-label--standalone">转视频镜头数</span><input className="text-input" type="number" min="1" max="60" value={Math.max(1, form.videoIntroCount)} onChange={(event) => onChange({ videoIntroCount: Math.max(1, Math.min(60, Number(event.target.value) || 5)) })} /></label> : null}<div><span className="field-label field-label--standalone">每镜生成时长</span><div className="segmented-control"><button type="button" disabled={!form.dynamicStoryboard} className={form.videoIntroDurationMode === "narration" ? "is-selected" : ""} onClick={() => onChange({ videoIntroDurationMode: "narration" })}>跟随配音</button><button type="button" disabled={!form.dynamicStoryboard} className={form.videoIntroDurationMode === "fixed" ? "is-selected" : ""} onClick={() => onChange({ videoIntroDurationMode: "fixed" })}>固定秒数</button></div></div>{form.dynamicStoryboard && form.videoIntroDurationMode === "fixed" ? <label><span className="field-label field-label--standalone">目标镜头时长（5–15 秒）</span><input className="text-input" type="number" min="5" max="15" value={Math.max(5, Math.min(15, form.videoIntroDuration))} onChange={(event) => onChange({ videoIntroDuration: Math.max(5, Math.min(15, Number(event.target.value) || 6)) })} /></label> : null}<p className="template-hint">已接入你自己的 RunningHub：静态分镜与真实配音完成后自动图片转视频，结果逐镜写回；海螺输出 6 秒，PixVerse 可按真实配音或固定目标生成 5–15 秒，草稿最终以真实配音时间线对齐。</p></div> : null}
      ...
      <section className="builder-card">
        ...
        {form.videoForm !== "podcast" && form.voiceSource === "tts" ? <><div className="field-group"><span className="field-label field-label--standalone">配音员</span><div className="segmented-control"><button type="button" className={form.ttsProvider === "volcengine" ? "is-selected" : ""} onClick={() => onChange({ ttsProvider: "volcengine", ttsVoiceId: "" })}>豆包</button><button type="button" className={form.ttsProvider === "minimax" ? "is-selected" : ""} onClick={() => onChange({ ttsProvider: "minimax", ttsVoiceId: "" })}>MiniMax</button></div></div><div className="field-group form-grid form-grid--two"><div><span className="field-label field-label--standalone">音色</span><div className="task-voice-select-row"><select aria-label="本任务音色" className="text-input" value={form.ttsVoiceId} onChange={(event) => onChange({ ttsVoiceId: event.target.value })}>{voices.map((voice) => <option key={voice.id} value={voice.id}>{voice.name} · {voice.tag}</option>)}</select><div className="task-voice-preview-actions"><button type="button" className="secondary-button" disabled={!selectedVoice || !hasTtsCredentials || Boolean(previewingVoiceId)} onClick={() => selectedVoice ? onPreviewVoice(selectedVoice.id) : undefined}>{previewingVoiceId === selectedVoice?.id && previewingVoiceMode === "short" ? "短句生成中…" : "短句试听"}</button><button type="button" className="secondary-button" disabled={!selectedVoice || !hasTtsCredentials || !form.inputText.trim() || Boolean(previewingVoiceId)} onClick={() => selectedVoice ? onPreviewVoice(selectedVoice.id, form.inputText) : undefined}>{previewingVoiceId === selectedVoice?.id && previewingVoiceMode === "full" ? "全文生成中…" : `全文试听（${form.inputText.trim().length} 字）`}</button></div></div></div><div><span className="field-label field-label--standalone">配音语速</span><div className="segmented-control">{speedPresets.map((speed) => <button key={speed.value} type="button" className={form.ttsSpeed === speed.value ? "is-selected" : ""} onClick={() => onChange({ ttsSpeed: speed.value })}>{speed.label} <small>{speed.value === 1 ? "1.0" : String(speed.value)}×</small></button>)}</div></div></div>{voicePreview ? <div className="task-voice-preview" aria-live="polite"><div><strong>{voicePreview.mode === "full" ? "全文试听" : "短句试听"} · {voicePreview.name}</strong><span>{form.ttsSpeed}× · {voicePreview.textLength} 字 · 临时试听，不写入正式配音</span></div><audio controls src={voicePreview.url} /></div> : null}{voicePreviewError ? <p className="task-voice-error">试听失败：{voicePreviewError}</p> : null}</> : null}
