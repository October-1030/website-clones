...
export function TaskWorkbench({ task, busy, voices, configuredVoiceId, voicePreview, previewingVoiceId, previewingVoiceMode, voicePreviewError, onPreviewVoice, onApplyTaskVoice, onTaskChange, onPause, onContinue, onCancel, onRunFromStep, onSaveArtifact, onRepairPromptAlignment, onRegenerateImage, onUploadImage, onUploadDynamicVideo, onBorrowImage, onRepairFailedImages, onRegenerateAudio, onUpdateImageCrop, onUpdateTimeline, onRepackDraft }: TaskWorkbenchProps) {
  ...
  const generatedVoiceNames = Array.from(new Set(task.media.audioSegments.map((segment) => voiceNameById.get(segment.voiceId) || segment.voiceId)));
  const voiceApplyTarget = voiceDiffersFromDefault ? configuredVoice : taskVoice;
  ...
  return (
    <section className="pipeline-panel" aria-live="polite">
      ...
      {task.media.audioSegments.length ? <div className="audio-workbench"><div className="artifact-editor__head"><div><strong>Step 6 · 配音与字幕时间线</strong><span>{task.media.audioSegments.filter((audio) => audio.status === "ready").length}/{task.media.audioSegments.length} 段 · {task.videoForm === "podcast" ? "A/B 双人声" : "原版逐镜 TTS · 实测时长对齐"}</span></div></div>{task.videoForm !== "podcast" && (voiceDiffersFromDefault || staleAudioSegments.length) ? <div className="audio-voice-mismatch" role="alert"><div><strong>当前成品仍是旧音色：{generatedVoiceNames.join("、")}</strong><span>目标音色：{voiceApplyTarget?.name || task.options.ttsVoiceId}。应用后会重配全部旁白并重建字幕与草稿。</span></div><button type="button" disabled={busy || !voiceApplyTarget} onClick={() => voiceApplyTarget ? onApplyTaskVoice(voiceApplyTarget.id) : undefined}>全部改成“{voiceApplyTarget?.name || "新音色"}”</button></div> : null}{task.media.audioSegments.map((audio) => <article key={audio.id}><span>#{audio.shotId} {audio.speaker ? `[${audio.speaker}]` : ""}</span><p>{audio.text}</p>{audio.url ? <audio controls src={audio.url} /> : <em>{audio.error}</em>}<small>{audio.durationSec.toFixed(1)}s · {voiceNameById.get(audio.voiceId) || audio.voiceId}</small><button type="button" disabled={busy} onClick={() => onRegenerateAudio(audio.shotId)}>重配本段</button></article>)}</div> : null}
