...
import { getCoverTemplate } from "../lib/cover-prompt";
...
const imageProviderLabels: Record<ImageProviderId, string> = {
  jimeng: "即梦",
  "all-purpose": "全能绘图",
  minimax: "MiniMax",
  "openai-compatible": "自定义平台",
};
...
function sopQualityChecks(task: StoryboundTask): SopCheck[] {
  ...
  // Before Step 2 writes a rewrite artifact, missing rewrite fields are
  // expected. They must not be presented as frozen, stale output.
  const rewriteBlockers = task.artifacts.rewrite ? blockingRewriteIssues(task) : [];
  ...
  const hasCover = task.media.coverImages.some((image) => image.status === "ready");
  checks.push(hasCover
    ? { level: "pass", label: "发布封面与首帧", detail: "发布海报按所选比例单独保存（默认 3:4）；草稿另生成 9:16 首帧代理图，在第 0 秒覆盖 1 帧。正片首镜、首句和字幕仍从 0 秒同步开始。" }
    : { level: "advice", label: "发布封面与首帧", detail: "可在创建任务时生成或上传封面。发布图独立保存；草稿会使用 9:16 代理图叠在第 0 秒，不从第 1 镜借图，也不占用旁白时间。" });
...
export function TaskWorkbench({ task, busy, voices, configuredVoiceId, voicePreview, previewingVoiceId, previewingVoiceMode, voicePreviewError, onPreviewVoice, onApplyTaskVoice, onTaskChange, onPause, onContinue, onCancel, onRunFromStep, onSaveArtifact, onRepairPromptAlignment, onRegenerateImage, onRegenerateCovers, onUploadImage, onUploadDynamicVideo, onBorrowImage, onRepairFailedImages, onRegenerateAudio, onUpdateImageCrop, onUpdateTimeline, onRepackDraft }: TaskWorkbenchProps) {
  ...
  const voiceNameById = new Map(voices.map((voice) => [voice.id, voice.name]));
  const staleAudioSegments = task.videoForm === "podcast"
    ? []
    : task.media.audioSegments.filter((segment) => segment.voiceId !== task.options.ttsVoiceId);
  const staleGeneratedVoiceNames = Array.from(new Set(staleAudioSegments.map((segment) => voiceNameById.get(segment.voiceId) || segment.voiceId)));
  ...
  return (
    <section className="pipeline-panel" aria-live="polite">
      ...
      {task.artifacts.prompts ? <div className="artifact-editor"><div className="artifact-editor__head"><div><strong>Step 4 · 原版绘图提示词</strong><span>{task.artifacts.prompts.templateVersion} · {task.artifacts.prompts.prompts.length} 条 · 默认 {imageProviderLabels[task.options.imageProvider || "minimax"]}</span></div><button type="button" className="primary-button" disabled={busy || rewriteBlockers.length > 0} onClick={() => onSaveArtifact(3)}>保存并开始出图</button></div><div className="prompt-editor-list">{task.artifacts.prompts.prompts.map((prompt, index) => <article key={prompt.shotId}><div className="prompt-editor-list__heading"><strong>第 {prompt.shotId} 镜</strong><div className="prompt-provider-actions"><select aria-label={`第 ${prompt.shotId} 镜图片引擎`} className="text-input prompt-provider-select" value={prompt.provider || ""} onChange={(event) => updatePrompt(index, { provider: event.target.value ? event.target.value as ImageProviderId : undefined })}><option value="">跟随任务 · {imageProviderLabels[task.options.imageProvider || "minimax"]}</option><option value="jimeng">即梦</option><option value="all-purpose">全能绘图</option><option value="minimax">MiniMax</option><option value="openai-compatible">自定义平台</option></select><button type="button" className={prompt.useReference ? "is-reference" : "is-environment"} onClick={() => updatePrompt(index, { useReference: !prompt.useReference })}>{prompt.useReference ? "主角参考图" : "纯场景 / 物件"}</button></div></div><textarea value={prompt.prompt} onChange={(event) => updatePrompt(index, { prompt: event.target.value })} /><input className="text-input" value={prompt.negativePrompt} onChange={(event) => updatePrompt(index, { negativePrompt: event.target.value })} /></article>)}</div></div> : null}

      {task.media.images.length ? <div className="pipeline-images"><div className="pipeline-images__head"><div><strong>Step 5 · 分镜图片工作台</strong><span>{task.media.images.filter((image) => image.status === "ready" || image.status === "borrowed").length}/{task.media.images.length} 张可用</span></div>{failedImages.length ? <button type="button" className="primary-button" disabled={busy} onClick={onRepairFailedImages}>只修复 {failedImages.length} 张失败图</button> : null}</div>{task.options.historicalPortraitMode ? <p className="historical-portrait-notice"><strong>历史人物真实性模式</strong>：{task.options.historicalPortraitNotice || "正脸镜头使用已核实历史照片；AI 只生成场景或不露脸镜头。"}</p> : null}<div className="pipeline-image-grid">{task.media.images.map((image) => {
        const crop = image.crop || { x: 0, y: 0, scale: 1 };
        const dynamicVideo = (task.media.videos || []).find((video) => video.shotId === image.shotId);
        return <article key={`${image.shotId}-${image.id}`} className={image.status === "failed" ? "is-failed" : ""}>{image.url ? <img src={image.url} alt={`第 ${image.shotId} 镜`} /> : <div className="image-failure">{image.error || "图片生成失败"}</div>}<div><strong>第 {image.shotId} 镜</strong><span>{image.historicalPortrait ? "真实历史肖像 · " : image.provider ? `${imageProviderLabels[image.provider as ImageProviderId] || image.provider} · ` : ""}{dynamicVideo ? dynamicVideo.status === "ready" ? `动态视频 ${dynamicVideo.durationSec.toFixed(1)}s` : "动态视频失败" : image.status === "borrowed" ? `借用 #${image.borrowedFrom}` : image.status}</span></div><div className="image-card-actions">{!image.historicalPortrait ? <button type="button" disabled={busy} onClick={() => onRegenerateImage(image.shotId)}>重画</button> : null}<label><input type="file" accept="image/*" onChange={(event) => event.target.files?.[0] && onUploadImage(image.shotId, event.target.files[0])} />替换图片</label><label><input type="file" accept="video/mp4,video/quicktime,video/webm" onChange={(event) => event.target.files?.[0] && onUploadDynamicVideo(image.shotId, event.target.files[0])} />{dynamicVideo?.status === "ready" ? "替换视频" : "图片转视频产物"}</label>{image.status === "failed" ? <button type="button" onClick={() => onBorrowImage(image.shotId)}>相邻补位</button> : null}<a href={image.url} download={`shot-${image.shotId}.jpg`}>下载</a></div><div className="crop-controls"><button type="button" disabled={busy} onClick={() => onUpdateImageCrop(image.shotId, { ...crop, x: Number((crop.x - 0.05).toFixed(2)) })}>←</button><button type="button" disabled={busy} onClick={() => onUpdateImageCrop(image.shotId, { ...crop, y: Number((crop.y + 0.05).toFixed(2)) })}>↑</button><button type="button" disabled={busy} onClick={() => onUpdateImageCrop(image.shotId, { x: 0, y: 0, scale: 1 })}>居中</button><button type="button" disabled={busy} onClick={() => onUpdateImageCrop(image.shotId, { ...crop, y: Number((crop.y - 0.05).toFixed(2)) })}>↓</button><button type="button" disabled={busy} onClick={() => onUpdateImageCrop(image.shotId, { ...crop, x: Number((crop.x + 0.05).toFixed(2)) })}>→</button><button type="button" disabled={busy} onClick={() => onUpdateImageCrop(image.shotId, { ...crop, scale: Number(Math.max(0.5, crop.scale - 0.1).toFixed(2)) })}>－</button><button type="button" disabled={busy} onClick={() => onUpdateImageCrop(image.shotId, { ...crop, scale: Number(Math.min(3, crop.scale + 0.1).toFixed(2)) })}>＋</button><span>{crop.scale.toFixed(1)}×</span></div><p title={image.prompt}>{image.prompt}</p>{image.sourceUrl ? <p className="stock-source"><span>{image.sourceTitle || "Wikimedia Commons"}</span><span>{image.attribution || image.license}</span><a href={image.sourceUrl} target="_blank" rel="noreferrer">核对来源</a>{image.licenseUrl ? <a href={image.licenseUrl} target="_blank" rel="noreferrer">查看许可</a> : null}</p> : null}{dynamicVideo?.status === "failed" ? <p className="dynamic-video-error">RunningHub：{dynamicVideo.error}</p> : null}</article>;
      })}</div></div> : null}

      {task.media.coverImages.length ? <div className="pipeline-images pipeline-covers"><div className="pipeline-images__head"><div><strong>独立封面海报</strong><span>{task.media.coverImages.length} 张 · 原版模板：{getCoverTemplate(task.options.coverTemplateId).name} · {task.options.imageProvider === "minimax" && task.options.coverMode === "titled" ? "MiniMax 底图 + 原版模板排字" : `${imageProviderLabels[task.options.imageProvider || "minimax"]} 直出`}</span></div><div className="image-card-actions"><button type="button" className="secondary-button" disabled={busy} onClick={onRegenerateCovers}>只重画封面</button><button type="button" className="secondary-button" disabled={busy} onClick={() => onRunFromStep(4)}>连同分镜重新出图</button></div></div><div className="pipeline-image-grid">{task.media.coverImages.map((image, index) => <article key={`${image.id}-${index}`} className={image.status === "failed" ? "is-failed" : ""}>{image.url ? <img src={image.url} alt={`封面 ${index + 1}`} /> : <div className="image-failure">{image.error || "封面生成失败"}</div>}<div><strong>封面 {index + 1}</strong><span>{image.status}</span></div><div className="image-card-actions"><a href={image.url} download={`cover-${index + 1}.jpg`}>下载</a></div><p title={image.prompt}>{image.prompt}</p></article>)}</div></div> : null}

      {task.media.audioSegments.length ? <div className="audio-workbench"><div className="artifact-editor__head"><div><strong>Step 6 · 配音与字幕时间线</strong><span>{task.media.audioSegments.filter((audio) => audio.status === "ready").length}/{task.media.audioSegments.length} 段 · {task.videoForm === "podcast" ? "A/B 双人声" : "原版逐镜 TTS · 实测时长对齐"}</span></div></div>{task.videoForm !== "podcast" && staleAudioSegments.length ? <div className="audio-voice-mismatch" role="alert"><div><strong>当前成品仍含旧音色：{staleGeneratedVoiceNames.join("、")}</strong><span>本任务音色：{taskVoice?.name || task.options.ttsVoiceId}。应用后会重配全部旁白并重建字幕与草稿。</span></div><button type="button" disabled={busy || !taskVoice} onClick={() => taskVoice ? onApplyTaskVoice(taskVoice.id) : undefined}>全部改成“{taskVoice?.name || "本任务音色"}”</button></div> : null}{task.media.audioSegments.map((audio) => <article key={audio.id}><span>#{audio.shotId} {audio.speaker ? `[${audio.speaker}]` : ""}</span><p>{audio.text}</p>{audio.url ? <audio controls src={audio.url} /> : <em>{audio.error}</em>}<small>{audio.durationSec.toFixed(1)}s · {voiceNameById.get(audio.voiceId) || audio.voiceId}</small><button type="button" disabled={busy} onClick={() => onRegenerateAudio(audio.shotId)}>重配本段</button></article>)}</div> : null}
