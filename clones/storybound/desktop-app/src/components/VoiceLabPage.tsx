...
export function VoiceLabPage({ config, credentialStatus, onChange, onOpenSettings }: VoiceLabPageProps) {
  ...
  const generate = async () => {
    if (!selectedVoice || busy) return;
    setBusy(true);
    setError("");
    try {
      const audio = await synthesizeTts({ provider, text, voiceId: selectedVoice.id, speed, config });
      ...
      setResults((items) => [{
        ...
        fileName: `配音_${selectedVoice.name}_${stamp}.mp3`,
        audioUrl,
  ...
  return (
    <div className="tts-page voice-lab-page">
      ...
      <section className="tts-card voice-lab-card">
        ...
        <div className="voice-chip-list">{availableVoices.map((voice) => <button className={voice.id === selectedVoice?.id ? "selected" : ""} key={voice.id} onClick={() => setVoice(voice.id)} type="button"><span aria-hidden="true">⌁</span>{voice.name}{voice.version ? <em>{voice.version}</em> : null}</button>)}<button className="more" onClick={onOpenSettings} type="button">更多音色…</button></div>
        ...
        <div className="voice-lab-actions">
          <button className="tts-primary" disabled={!text.trim() || busy || !selectedVoice || !hasCredentials} onClick={generate} type="button">♩ {busy ? "正在合成…" : "生成配音"}</button>
