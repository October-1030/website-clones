...
if (!taskId) throw new Error("Usage: node scripts/regenerate-environment-images.mjs <task-id> [batch-size] [shot-id,shot-id]");
...
function isEnvironmentPrompt(prompt) {
  return prompt?.useReference !== true && prompt?.use_reference !== true && prompt?.characterAction !== true;
}

async function main() {
  let task = (await requestJson(`/api/tasks/${encodeURIComponent(taskId)}`)).task;
  const prompts = (task.artifacts?.prompts?.prompts || []).filter((prompt) => isEnvironmentPrompt(prompt) && (!selectedIds.size || selectedIds.has(Number(prompt.shotId))));
  if (!prompts.length) throw new Error("任务没有纯场景分镜");
  for (let start = 0; start < prompts.length; start += batchSize) {
    ...
    process.stdout.write(`已重画纯场景 ${start + 1}-${start + batch.length}/${prompts.length}：${batch.map((item) => item.shotId).join(", ")}\n`);
  }
}
