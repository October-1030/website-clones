...
基于 Storybound 1.17.0 客户端证据实现的独立本地工作台。它不会连接原产品的创作、授权或计费后台；LLM、图片和 TTS 使用用户自己的 API。
...
- 对标账号同步按原版分页契约工作：刷新最新 15 条、加载更多 15 条、可停止的连续历史加载，并保留本地转写/分析
- Storybound 1.17.0 字幕行号、单行字数限制和超字标红
...
“单视频解析”使用从原客户端 1.17.0 公开代码确认的数据路径，可解析公开分享链接并返回标题、作者、封面、清晰度和可下载媒体；结果可保存到本地资料库。保存后打开作品，点击“一键提取公开视频文案”，服务会重新解析媒体、下载到临时目录并用本机 ASR 转写，完成后自动删除临时媒体。

原版的“添加账号／刷新全部作品”是两段式数据链路：先用视频号分享链接识别远端账号 ID，再用该 ID 按游标分页拉取作品。独立版支持两种不依赖微信登录态的数据源：Just One API（推荐，可先用有限免费测试额度）和大家啦数据（原版同类接口）。两者都不读取微信 Cookie、不安装证书、不修改系统代理。

第一次只需配置一次：

1. 打开“系统设置 → 对标数据”。
2. 推荐先在 [Just One API](https://justoneapi.com/) 注册并复制 Token；也可以在[大家啦数据官网](https://www.dajiala.com/home/)复制 Key。
3. 回到本地网页选择服务商并粘贴 Token/Key。Just One Token 首次识别账号时真实验证；大家啦 Key 保存前会读取余额验证。
4. 打开“对标监控”，添加任意一个目标账号的视频号分享链接，再点击刷新。“加载更多”和“连续加载”会沿分页游标继续拉取历史作品。

账号识别、刷新和翻页可能由数据提供方按次计费（Just One API 提供有限免费测试额度），页面会在执行前提示；已拉取的作品、文案和分析结果保存在本地。Token/Key 只写入被 Git 忽略的 `desktop-app/.storybound-data/benchmark-source.json`，不会返回浏览器，也不会上传到仓库；可随时在设置页删除。

也可以通过环境变量配置 Just One API：

```powershell
$env:STORYBOUND_JUSTONE_API_TOKEN = "你的 Token"
npm run dev
```

如果你已经合法拥有原 Storybound 的授权参数，仍可使用兼容回退模式：
...
`smoke:task` 验证任务持久化、真实音频时长、裁切参数、剪映媒体轨道和 ZIP；`smoke:media` 复用已有本地素材，验证双分镜 H.264/AAC MP4 与剪映包，不消耗生图或 TTS 额度；`smoke:pipeline` 会使用本机 LLM 凭据真实跑一遍 1.17.0 文本提示词链，因此会产生少量 API 用量。
