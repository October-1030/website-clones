...
原版的“添加账号／刷新全部作品”是两段式数据链路：先用视频号分享链接识别远端账号 ID，再用该 ID 按 15 条一页拉取作品。独立版已经按相同契约接入“大家啦数据”的公开视频号接口，不再要求绑定原 Storybound 邮箱或设备指纹。

第一次只需配置一次：

1. 打开“系统设置 → 对标数据”。
2. 在[大家啦数据官网](https://www.dajiala.com/home/)注册或登录，复制控制台显示的 Key；没有单独设置验证码时，验证码框留空。
3. 回到本地网页粘贴 Key，点击“保存并测试”。页面会先读取余额验证密钥，验证通过才在本机保存。
4. 打开“对标监控”，添加任意一个目标账号的视频号分享链接，再点击“刷新最新 15 条”。“加载更多”和“连续加载”会沿分页游标继续拉取历史作品。

账号识别、刷新和翻页由数据提供方按次计费，页面会在执行前明确提示；已拉取的作品、文案和分析结果保存在本地。Key 只写入被 Git 忽略的 `desktop-app/.storybound-data/benchmark-source.json`，不会返回浏览器，也不会上传到仓库；可随时在设置页删除。
