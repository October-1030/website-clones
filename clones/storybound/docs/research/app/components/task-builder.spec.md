...
- TTS：MiniMax/火山、音色、语速；原客户端可见档位为 `0.85× / 1.0× / 1.15× / 1.3×`，其中 `1.0×` 明确标为默认。默认“原版复刻模式”逐镜生成，每个分镜独立图片、独立 TTS、独立字幕，按生成后实测的 MP3 时长一一对齐。
- 音色选择器旁必须提供短句试听与全文试听：短句用于低成本挑音色，全文按当前语速和本任务实际正文生成临时音频。任务创建后全文优先读取 Step 2 最终改写正文，其次读取预审正文和原文；试听不得推进流水线、覆盖 Step 6 或写入正式配音。任务创建后仍显示本任务音色；若系统设置默认音色后来发生变化，显示差异并提供“试听系统默认 / 应用到本任务”，不得悄悄覆盖已保存任务。
- 系统音色库与默认 voice ID 刷新后保持；仅保存非秘密偏好和音色元数据，禁止将 API Key、Token 或 App ID 写入浏览器存储。
