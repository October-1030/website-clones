...
| 对标账号分页同步契约 | 通过：最新 15 条、下一页 15 条、连续补齐、可停止 |
| 对标账号分页去重与本地字段保留 | 通过：33 条三页数据重复刷新后仍为 33 条，本地转写未被覆盖 |
...
- 原版 `刷新本账号` 只请求最新一页（最多 15 条），不会一次拉完全部历史。
- 原版历史作品通过 `last_buffer` 分页；“加载更多”请求一页，“连续加载”逐页请求且每页可能扣除积分。
- 独立复刻已实现这三条路径，并在运行时显示已保存数量、已载入页数和是否还有历史页。
- 本地模拟源验收使用 15 + 15 + 3 条三页数据；验证了游标前进、末页停止、运行中停止、去重合并，以及本地转写/分析字段不被远端刷新覆盖。
- 本机原客户端未发现已绑定邮箱状态，因此没有用真实账号接口消耗积分。当前 `127.0.0.1:5173` 会明确显示“账号作品同步：未配置”并禁用三项同步操作；MiniMax API 不承担账号作品同步。
