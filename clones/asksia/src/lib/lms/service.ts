...
async function loadConnection(context: AuthenticatedCloud, connectionId: string): Promise<LmsConnectionRecord> {
  const { data, error } = await context.client
    ...
    .single();
  if (error || !data) throw new LmsError("Canvas connection was not found.", "lms_connection_not_found", 404);
...
export async function syncLmsConnection(
  ...
  try {
    const token = decryptLmsToken(connection.access_token_ciphertext, options.environment);
    const canvas = new CanvasClient(connection.instance_url, token, { fetchImpl: options.fetchImpl });
    await canvas.verifyConnection();
    ...
    for (const course of courses) {
      const { data: savedCourse, error: courseError } = await context.client
        ...
        .select("id")
        .single();
      if (courseError || !savedCourse) throw databaseFailure("Unable to save a Canvas course.");

      const snapshot = await canvas.loadCourseSnapshot(course);
      ...
      for (const values of chunk(rows)) {
        ...
        if (error) throw databaseFailure("Unable to save Canvas materials.");
      }
      ...
      if (existingError) throw databaseFailure("Unable to reconcile Canvas materials.");
      ...
      for (const ids of chunk(staleIds)) {
        ...
        if (error) throw databaseFailure("Unable to remove stale Canvas materials.");
      }
      coursesSynced += 1;
    ...
    if (completeError || connectionError) throw databaseFailure("Canvas sync completed but status could not be saved.");
    return { runId: String(run.id), coursesSynced, materialsSynced, warnings };
  } catch (error) {
    const failure = error instanceof LmsError
      ? error
      : new LmsError("Canvas sync failed.", "lms_sync_failed", 502);
    await Promise.all([
      context.client
        .from("lms_sync_runs")
        .update({
      ...
      context.client
        .from("lms_connections")
        .update({
          status: failure.code === "canvas_token_rejected" ? "expired" : "error",
